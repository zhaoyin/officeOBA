...
namespace AODL.Document.Export.Html
{
	...
	public class OpenDocumentHtmlExporter : IExporter, IPublisherInfo
	{
		...
		{
			this._exporterror				= new List<AODLWarning>();

            this._supportedExtensions = new List<DocumentSupportInfo>();
		...
		private IList<DocumentSupportInfo> _supportedExtensions;
		/// <summary>
        ...
        private IList<AODLWarning> _exporterror;
		/// <summary>
			...
			try
			{
				...
				foreach(Graphic graphic in document.Graphics)
				{
					if (graphic.GraphicFile != null)
					{
						...
						{
                            if (!File.Exists(Path.Combine(picturedir, graphic.GraphicFile.Name)))
                                graphic.GraphicFile.CopyTo(File.OpenWrite(Path.Combine(picturedir, graphic.GraphicFile.Name)));
						}
						else
                            graphic.GraphicFile.CopyTo(File.OpenWrite(Path.Combine(picturedir, graphic.GraphicFileName)));
                    }					
				}
			}
			catch(Exception ex)
