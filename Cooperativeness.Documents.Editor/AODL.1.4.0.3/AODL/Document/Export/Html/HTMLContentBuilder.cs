...
namespace AODL.Document.Export.Html
{
	...
	public class HTMLContentBuilder
	{
			...
			try
			{
				if (iTextCollection != null)
				{
					foreach(IText iText in iTextCollection)
					{
						...
						{
							string textContent	= ((XText)iText.Node).Value;
		...
		/// </summary>
		/// <param name="mixedContent">list of objects. The objects could be
		/// IContent or IText.</param>
		/// <param name="paragraphStyle">The paragraph style.</param>
		/// <returns></returns>
		public string GetMixedContentAsHTML(IList<IContent> mixedContent, ParagraphStyle paragraphStyle)
		{
			string html					= "";
			int tabStopCnt				= 0;
			...
			try
			{
				if (mixedContent != null)
				{
					foreach(IContent ob in mixedContent)
					{
						...
						if (ob is SimpleText)
						{
							html			+= this.ReplaceControlNodes(((XText)((IText)ob).Node).Value);
						}
		...
		public string GetTableOfContentsAsHtml(TableOfContents tableOfContents)
		{
			...
			try
			{
				...
				{
				    XElement nodeTitle = tableOfContents.Node
                        .Elements(Ns.Text + "index-body")
                        .Elements(Ns.Text + "index-title")
                        .Elements(Ns.Text + "p")
                        .FirstOrDefault();
					if (nodeTitle != null)
					{
						html			+= "<p "+this.HTMLStyleBuilder.HeaderHtmlStyles[0]+">\n";
						html			+= nodeTitle.Value;
						html			+= "\n</p>\n";
					}
			...
			if (reference.Document is TextDocument)
			{
				...
				try
				{
					...
					if (control is ODFListBox)
					{
						...
						{
							string selected = "";
							if (o.Selected.HasValue && o.Selected.Value) selected = " selected";
		...
		public string GetUnknowContentAsHtml(UnknownContent unknownContent)
		{
			...
			try
			{
				if (unknownContent != null)
				{
					...
					{
						foreach(XElement node in ((XElement)unknownContent.Node).Elements())
							if (node.Value != null)
								html	+= this.ReplaceControlNodes(node.Value+" ");
					}
				}
			}
			catch(Exception ex)
			...
			try
			{
				if (unknownTextContent != null)
				{
					if (unknownTextContent.Node != null)
						html			+= this.ReplaceControlNodes(unknownTextContent.Node.Value);
				}
			}
			catch(Exception ex)
			{
