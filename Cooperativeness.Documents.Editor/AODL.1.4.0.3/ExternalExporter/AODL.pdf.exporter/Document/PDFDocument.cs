...
namespace AODL.ExternalExporter.PDF.Document
{
	...
	public class PDFDocument
	{
        ...
        {
            DoExport(document, new FileStream(fileName, FileMode.Create));
        }
		...
		public void DoExport(IDocument document, Stream stream)
		{
			try
			{
				this.LoadDefaultStyles(document);
				this.CreatePDFDocument(stream);
		...
		private void CreatePDFDocument(Stream stream)
		{
			try
			{
				...
				PdfWriter pdfWriter = PdfWriter.GetInstance(this._document, stream);
				this._document.Open();
			}
			catch(Exception)
			{
