...
namespace Crownwood.DotNetMagic.Docking
{
    ...
    public class TabStub : UserControl
    {
        ...
        /// <param name="stubsShowAll"></param>
        /// <param name="dockManager"></param>
        public TabStub(VisualStyle style, bool stubsShowAll,DockingManager dockManager):this(style,stubsShowAll)
        {
            this.dockingManager = dockManager;
        }
        ...
        private void DrawOutlineForCell(Graphics g, Pen pen, Rectangle rect)
        {
			...
			else if (_style == VisualStyle.IDE2005)
			{
                ...
                if (this.dockingManager.Customize)
                {
                    using (Brush fillBrush = new LinearGradientBrush(new Rectangle(fillRect.X - 1, fillRect.Y - 1, fillRect.Width + 2, fillRect.Height + 2), this.dockingManager.BackColor, _colorDetails.BaseColorStub, AngleForEdge()))
