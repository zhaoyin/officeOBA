...
namespace Crownwood.DotNetMagic.Controls
{
    ...
    public class TabControl : Panel
    {
        ...
        #region weijz add to add closeImage to TagPage
        private bool _showTabPageClose;		    // should the TabPage close button be displayed
        /// <summary>
        /// 页签上的关闭按钮
        /// </summary>
        private IDE2005CloseButton _tabPageCloseButton;    

        /// <summary>
        /// 是否显示页签(TabPage)上的关闭按钮
        /// </summary>
        [Category("Appearance")]
        public bool ShowTabPageClose
        {
            get
            {
                return this._showTabPageClose;
            }
            set
            {
                if (this._showTabPageClose != value)
                {
                    this._showTabPageClose = value;
                    _recalculate = true;                    
                    Invalidate();
                }
            }
        ...
        public TabControl()
        {
            ...
            _tabPageCloseButton = new IDE2005CloseButton(this);
            ...
            _tabPageCloseButton.Office2003GradBack = false;
            ...
            _tabPageCloseButton.AlwaysDrawEnabled = true;
            ...
            _tabPageCloseButton.Image = _internalImages.Images[(int)ImageStrip.Close];
            ...
            _tabPageCloseButton.Click += new EventHandler(OnCloseButton);

            // Set their fixed sizes
            _activeArrow.Size = _leftArrow.Size = _rightArrow.Size = _closeButton.Size = _tabPageCloseButton.Size = new Size(BUTTON_WIDTH, BUTTON_HEIGHT);

            // Add child controls
            Controls.AddRange(new Control[]{_closeButton, _leftArrow, _rightArrow, _activeArrow,_tabPageCloseButton});

            _showTabPageClose = false;
            this._tabPageCloseButton.Visible = false;                       
        ...
        protected override void Dispose( bool disposing )
        {
            if(disposing)
            {
				...
				if (_ia != null)
				{
                    ...
                    _tabPageCloseButton.ImageAttributes = null;
					_ia.Dispose();
					_ia = null;
				}
                ...
                if (_style != value)
                {
					...
					switch(_style)
					{
						...
						case VisualStyle.Plain:
                            ...
                            _tabPageCloseButton.Style = VisualStyle.Plain;
							break;
						case VisualStyle.Office2003:
                            ...
                            _tabPageCloseButton.Style = VisualStyle.Office2003;
							break;
						case VisualStyle.IDE2005:
                            ...
                            _tabPageCloseButton.Style = VisualStyle.IDE2005;
							break;
					}
        ...
        protected virtual void Recalculate()
        {
			...
			if (!_leftMouseDownDrag)
			{
                ...
                //李正修改的。foreach循环在快速关闭的时候，会报集合已经改变的异常，参见cqU8dp200222995
                ...
                // Position each page control correctly
                int iPagecount = _tabPages.Count ;
                for (int iindex = iPagecount; iindex > 0;iindex -- )
                {
                    TabPage page = _tabPages[iindex-1];
                    if (page.Control != null)
                        page.Control.SetBounds(pageLoc.X, pageLoc.Y, pageSize.Width, pageSize.Height);
                    else
                        page.SetBounds(pageLoc.X, pageLoc.Y, pageSize.Width, pageSize.Height);
        ...
        protected virtual void RecalculateButtons()
        {
			...
			if ((_style == VisualStyle.Office2003) || (_style == VisualStyle.IDE2005))
			{
                ...
                _tabPageCloseButton.BackColor = _activeArrow.BackColor = _closeButton.BackColor = _leftArrow.BackColor = _rightArrow.BackColor = backColor;                 
			}
			else
			{
				if ((_appearance == VisualAppearance.MultiBox) || (_style == VisualStyle.Plain))
                   _tabPageCloseButton.BackColor = _activeArrow.BackColor = _closeButton.BackColor = _leftArrow.BackColor = _rightArrow.BackColor = Color.Transparent;
				else
                   _tabPageCloseButton.BackColor = _activeArrow.BackColor = _closeButton.BackColor = _leftArrow.BackColor = _rightArrow.BackColor = _backIDE;
			}

            if (this._showTabPageClose)
            {
        ...
        /// 
        /// </summary>
        protected void DisplayTabCloseButton(bool show)
        {
            if (show)
            {
                if (this.SelectedTab != null)
                {
                    Rectangle rectTab = (Rectangle)_tabRects[_tabPages.IndexOf(this.SelectedTab)];
                    int xMax = rectTab.Right - _position[_styleIndex, (int)PositionIndex.BorderRight];
                    int xDraw = xMax - _tabPageCloseButton.Image.Width - _position[_styleIndex, (int)PositionIndex.ImageGapLeft];
                    int yDraw = rectTab.Top + (rectTab.Height - _tabPageCloseButton.Image.Height) / 2;
                    this._tabPageCloseButton.Location = new Point(xDraw, yDraw);
                    this._tabPageCloseButton.rect = rectTab;
                    this._tabPageCloseButton.Show();
                }
                else
                {
                    this._tabPageCloseButton.Hide();
                }
            }
            else
            {
        ...
        /// 
        /// </summary>
        protected virtual void DisplayTabCloseButton()
        {
        ...
        protected virtual int GetTabPageSpace(Graphics g, TabPage page)
        {
            ...
            #region weijz 2009-03-12 
            if (_showTabPageClose)
            {
                width += _position[_styleIndex, (int)PositionIndex.BorderLeft] +
                         _position[_styleIndex, (int)PositionIndex.BorderRight];
            ...
            if (_appearance != VisualAppearance.MultiBox)
            {
                ...
                switch(_style)
                {
					...
					case VisualStyle.IDE2005:
                        ...
                        if (this.customize)
                            border = ColorCustomize.BorderColor;
        ...
        protected virtual void DrawTab(TabPage page, Graphics g, bool highlightText)
        {
            ...
            if (this._showTabPageClose)
            {
                xMax = xMax - BUTTON_WIDTH;                
            }
            //DrawTabCloseImage(rectTab,page, g, ref xMax);
        ...
        protected virtual void DrawIDE2005TabBorder(TabPage page, Graphics g, Rectangle rectPage)
        {
            if ((rectPage.Width > 0) && (rectPage.Height > 0))
            {
                ...
                using (Pen borderPen = new Pen(b), selectedBottomBorderPen = new Pen(this.BackColor))
                {
                    ...
                    if (page.Selected)
                        if (!this.customize)
                            ...
                            fillBrush = new LinearGradientBrush(fillRect, ColorCustomize.TabPageActiveStartColor, ColorCustomize.TabPageActiveEndColor, 90f);
                    else
                        if (!this.customize)
                            fillBrush = new LinearGradientBrush(fillRect, iat, iab, 90f);
                        else
                            fillBrush = new LinearGradientBrush(fillRect, ColorCustomize.TabPageInactiveStartColor, ColorCustomize.TabPageInactiveEndColor, 90f);
                    ...
                    if (_positionAtTop)
                    {
                        ...
                        if (page.Selected)
                        {
                            if (this._tabPages.IndexOf(page) != 0)
                                g.DrawLine(selectedBottomBorderPen, rectPage.Left, rectPage.Bottom, rectPage.Right, rectPage.Bottom);
                            else
                                g.DrawLine(selectedBottomBorderPen, rectPage.Left + 1, rectPage.Bottom, rectPage.Right, rectPage.Bottom);
                        }
                    }
                    else
                    {
                    ...
                    if (_tabsAreaRect.Contains(mousePos))
                    {
                        ...
                        if (!ce.Cancel)
                        {
                            if (_contextMenu != null)
                            {
                                ...
                                foreach (MenuCommand menucommand in _contextMenu.MenuCommands)
                                {

                                    if (menucommand.Text == "-")
                                    {
                                        ToolStripSeparator menuitem = new ToolStripSeparator();
                                        menuitem.Text = menucommand.Text;
                                        menuitem.Visible = menucommand.Visible;
                                    ...
                                    else
                                    {
                                        ...
                                        menuitem.Text = menucommand.Text;
                                        menuitem.ShortcutKeys = (System.Windows.Forms.Keys)menucommand.Shortcut;// Keys.Control | Keys.Shift | Keys.C;//menucommand.Shortcut  ;
                                        menuitem.Visible = menucommand.Visible;
                                        menuitem.Checked = menucommand.Checked;
                                        menuitem.Image = menucommand.Image;
                                        ContextMenuStrip.Items.Add(menuitem);
                                        menuitem.Tag = menucommand;

                                        menuitem.Click += new EventHandler(menuitem_Click);
                                        //menuitem.Click = menucommand.Click;
                                    }

                                    //menuitem.Click +=new EventHandler(menucommand.Click );
        ...
        {
            MenuCommand mnu = (sender as ToolStripItem).Tag as MenuCommand;
            mnu.PerformClick();
        }
        ...
        protected override void OnMouseMove(MouseEventArgs e)
        {
            ...
            if (_leftMouseDown)
            {
				...
				if (!_leftMouseDownDrag)
                {
                    ...
                    {
                        #region startDrag && !_ignoreDownDrag
		...
		private void DefineButtonImages()
		{
            ...
            _tabPageCloseButton.ImageAttributes = _ia;
		}
    ...
    /// Derived class to draw the background to match the caption.
    /// </summary>
    internal class IDE2005CloseButton : ButtonWithStyle
    {
        /// <summary>
        /// Initialize a new instance of the IDE2005CloseButton class.
        /// </summary>
        /// <param name="tc"></param>
        public IDE2005CloseButton(TabControl tc):base()
        {
            this.tabControl = tc;
        }
        ...
        protected override void OnPaintBackground(PaintEventArgs pevent)
        {
            ...
            Color caption1;
            Color caption2;
            if (tabControl.Customize)
            {
                caption1 = ColorCustomize.TabPageActiveStartColor;
                caption2 = ColorCustomize.TabPageActiveEndColor;
            }
            else
            {
                caption2 = caption1 = SystemColors.Control;                
            }
