...
namespace Crownwood.DotNetMagic.Controls
{
    ...
    public class TabbedGroups : UserControl, ISupportInitialize
	{
        ...
        #region weijz add 2009-04-24        
        private string _closeAllMenuText;
        private string _closeOtherMenuText;
        #endregion
        ...
        /// <summary>
        /// Represents the method that will handle the PageClosed event.
        /// weijz add 2010.07-08 处理关闭页签后执行的事件，比如释放对象
        /// </summary>
        public delegate void PageClosedHandler(TabbedGroups tg, TGCloseRequestEventArgs e);
        ...
        /// <summary>
        /// Occurs when a request to close all TabPage is being made.
        /// </summary>
        public event PageCloseRequestHandler PageCloseAllRequest;
        /// <summary>
        /// Occurs when a page close request is not canced and after it is removed from it's parent
        /// weijz add 2010-07-08
        /// </summary>
        public event PageClosedHandler PageClosed;

        /// <summary>
        /// Occurs when a request to close other TabPage is being made.
        /// </summary>
        public event PageCloseRequestHandler PageCloseOtherRequest;
        ...
        private void InternalConstruct(VisualStyle style)
		{
            ...
            ResetAtLeastOneLeaf();
            ResetCloseMenuText();
            ResetCloseAllMenuText();
            ResetCloseOtherMenuText();
        ...
        /// <summary>
        /// Gets or sets the text used for the CloseAll menu option.
        /// </summary>
        [Localizable(true)]
        [Category("Text String")]
        [DefaultValue("&CloseAll")]
        public string CloseAllMenuText
        {
            get { return _closeAllMenuText; }
            set { _closeAllMenuText = value; }
        }

        /// <summary>
        /// Resets the CloseAllMenuText property to its default value.
        /// </summary>
        public void ResetCloseAllMenuText()
        {
            CloseAllMenuText = "&CloseAll";
        }

        /// <summary>
        /// Gets or sets the text used for the CloseOther menu option.
        /// </summary>
        [Localizable(true)]
        [Category("Text String")]
        [DefaultValue("&CloseAll")]
        public string CloseOtherMenuText
        {
            get { return _closeOtherMenuText; }
            set { _closeOtherMenuText = value; }
        }

        /// <summary>
        /// Resets the CloseAllMenuText property to its default value.
        /// </summary>
        public void ResetCloseOtherMenuText()
        {
            CloseOtherMenuText = "&CloseOther";
        }
        ...
        /// <summary>
        /// Raises the PageCloseAllRequested event.
        /// </summary>
        /// <param name="e">A TGCloseRequestEventArgs that contains the event data.</param>
        public virtual void OnPageCloseAllRequested(TGCloseRequestEventArgs e)
        {
            // Has anyone registered for the event?
            if (PageCloseAllRequest != null)
                PageCloseAllRequest(this, e);
        }

        /// <summary>
        /// Raises the PageCloseOtherRequested event.
        /// </summary>
        /// <param name="e">A TGCloseRequestEventArgs that contains the event data.</param>
        public virtual void OnPageCloseOtherRequested(TGCloseRequestEventArgs e)
        {
            // Has anyone registered for the event?
            if (PageCloseOtherRequest != null)
                PageCloseOtherRequest(this, e);
        }
        ...
        /// </summary>
        /// <param name="e">A TGCloseRequestEventArgs that contains the event data.</param>
        public virtual void OnPageClosed(TGCloseRequestEventArgs e)
        {
            // Has anyone registered for the event?
            if (PageClosed != null)
                PageClosed(this, e);
		...
		public virtual void OnResizeFinish(TabGroupSequence tgs)
		{
            ...
            //
            // 在开发U870及U870SPX版本U8门户时，
            // OnResizeStart和OnResizeFinish两次Resize会引发门户闪烁，所以禁止OnResizeFinish
            // modified by: Edgard Zhou 
            // 
        ...
        internal void OnCloseAll(object sender, EventArgs e)
        {
            TabGroupLeaf currentLeaf = this.ActiveLeaf;
            if (currentLeaf != null)
            {
                TabControl _tabControl = currentLeaf.TabControl;
                // Is there anything to close? (or the control has been set to generate event always)
                if ((_tabControl.TabPages.Count > 0) || this.PageCloseWhenEmpty)
                {

                    TGCloseRequestEventArgs tge = new TGCloseRequestEventArgs(currentLeaf, _tabControl, _tabControl.SelectedTab);
                    // Generate event so handlers can perform appropriate action
                    this.OnPageCloseAllRequested(tge);

                    // Still want to close the page? (and there is something to close)
                    if (!tge.Cancel && (_tabControl.TabPages.Count > 0))
                    {
                        // Remove any prominent setting
                        if (this.ProminentLeaf != null)
                            this.ProminentLeaf = null;
        ...
        private bool ClosePage(TabGroupLeaf leaf, TabPage page)
        {
            bool pageClosed = false;
            // Is there anything to close? (or the control has been set to generate event always)
            if ((leaf.TabControl.TabPages.Count > 0) || this.PageCloseWhenEmpty)
            {
                TGCloseRequestEventArgs tge = new TGCloseRequestEventArgs(leaf, leaf.TabControl, page);

                // Generate event so handlers can perform appropriate action
                this.OnPageCloseRequested(tge);

                // Still want to close the page? (and there is something to close)
                if (!tge.Cancel && (leaf.TabControl.TabPages.Count > 0))
                {
                    // Remove any prominent setting
                    if (this.ProminentLeaf != null)
                        this.ProminentLeaf = null;

                    leaf.TabControl.TabPages.Remove(page);
                    pageClosed = true;
                }
            }
            return pageClosed;
        ...
        private void ClosePage(bool closeAll)
        {
            TabGroupLeaf activeLeaf = this.ActiveLeaf;
            TabPage activePage = this.ActiveTabPage;
            try
            {
                ...
                if (closeAll)
                {
                    pageClosed = ClosePage(activeLeaf, activePage);
                }

                if (pageClosed)
                {
                    for (int i = activeLeaf.TabPages.Count - 1; i >= 0; i--)
                    {
                        TabPage page = activeLeaf.TabPages[i];                        
                        if (activePage != page)                        
                        {
                            activeLeaf.TabControl.SelectedTab = page;
                            pageClosed = ClosePage(activeLeaf, page);
                            if (pageClosed == false)
                                break;
                        }
                    }

                    TabGroupLeaf currentLeaf = this.FirstLeaf();
                    while (currentLeaf != null && pageClosed)
                    {
                        if (currentLeaf != activeLeaf)
                        {
                            this.ActiveLeaf = currentLeaf;
                            for (int i = currentLeaf.TabPages.Count - 1; i >= 0; i--)
                            {
                                TabPage tabPage = currentLeaf.TabPages[i];
                                currentLeaf.TabControl.SelectedTab = tabPage;
                                pageClosed = ClosePage(currentLeaf, tabPage);
                                if (pageClosed == false)
                                    break;
                            }                            
                        }
                        currentLeaf = this.NextLeaf(currentLeaf);
                    }
                }
            }
            catch (Exception e)
        ...
        internal void OnCloseOther(object sender, EventArgs e)
        {
            TabGroupLeaf currentLeaf = this.ActiveLeaf;
            if (currentLeaf != null)
            {
                ...
                if ((_tabControl.TabPages.Count > 0) || this.PageCloseWhenEmpty)
                {
                    ...
                    TGCloseRequestEventArgs tge = new TGCloseRequestEventArgs(currentLeaf, _tabControl, _tabControl.SelectedTab);
                    // Generate event so handlers can perform appropriate action
                    this.OnPageCloseOtherRequested(tge);
